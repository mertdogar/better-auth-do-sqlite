...
export class LibSQLHttpServer {
   ...
   */
  private parseNamedArgs(namedArgs?: Record<string, SqlValue> | HranaValue[] | NamedArg[]): Record<string, SqlValue> | undefined {
    ...
    if (Array.isArray(namedArgs)) {
      ...
      if (namedArgs.length > 0 && namedArgs[0] && typeof namedArgs[0] === "object" && "name" in namedArgs[0]) {
  ...
  async handleRequest(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (method === "GET" && path === "/version") {
        return new Response(
          JSON.stringify({ version: "libsql-do-http-0.1.0" }),
          {
            status: 200,
  ...
  private async handleV1Batch(request: Request): Promise<Response> {
    try {
      ...
      return new Response(
        JSON.stringify({ error: error.message || "Query execution failed" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
  ...
  private async handlePipeline(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (!requests || !Array.isArray(requests)) {
        return new Response(
          JSON.stringify({ error: "Invalid request: requests array required" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          }
      ...
      if (baton === null || baton === undefined) {
        ...
        if (!existingStream) {
          return new Response(
            JSON.stringify({ error: "Invalid or expired baton" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            }
      ...
      return new Response(
        JSON.stringify({ error: error.message || "Pipeline execution failed" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
  ...
  private async handleSequence(
    ...
    const sql = request.sql_id !== null && request.sql_id !== undefined
  ...
  private async executeQuery(query: Query): Promise<{
    ...
    const rowArrays = rows.map((row: any) =>
      columns.map((col) => this.convertToSqlValue(row[col]))
    );
